...
Supports extracting text, markdown, and images from URLs."""
    ...
    async def execute(
        self, url: str, extract_depth: str = "basic", include_images: bool = False
    ) -> ToolResult:
        ...
        if not url:
            return ToolResult(status=ToolResultStatus.ERROR, error="No URL provided.")
        ...
        try:
            ...
            return ToolResult(
                status=ToolResultStatus.ERROR, error=f"Extraction failed: {str(e)}"
            )
    ...
    def _format_results(self, response: dict, include_images: bool) -> str:
        ...
        lines.append(
            f"## Extracted Content ({len(results)} successful, {len(failed)} failed)"
        )
        lines.append("")
