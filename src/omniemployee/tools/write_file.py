...
For existing files, can replace entire content or specific line ranges."""
    ...
    async def execute(
        ...
        try:
            ...
            if mode == "overwrite":
                ...
                result = await self._write_replace_lines(file_path, content, start_line, end_line)
            else:
                return ToolResult(
                    status=ToolResultStatus.ERROR,
    ...
    async def _write_insert(self, path: Path, content: str, line_num: int) -> ToolResult:
