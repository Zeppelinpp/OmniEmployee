...
MOCK_AIRPORTS = {
    "Beijing": {"code": "PEK", "name": "Beijing Capital International Airport", "name_cn": "北京首都国际机场"},
    "Shanghai": {"code": "PVG", "name": "Shanghai Pudong International Airport", "name_cn": "上海浦东国际机场"},
    "Guangzhou": {"code": "CAN", "name": "Guangzhou Baiyun International Airport", "name_cn": "广州白云国际机场"},
    "Shenzhen": {"code": "SZX", "name": "Shenzhen Bao'an International Airport", "name_cn": "深圳宝安国际机场"},
    "Tokyo": {"code": "NRT", "name": "Narita International Airport", "name_cn": "东京成田国际机场"},
    "Seoul": {"code": "ICN", "name": "Incheon International Airport", "name_cn": "首尔仁川国际机场"},
    "Singapore": {"code": "SIN", "name": "Singapore Changi Airport", "name_cn": "新加坡樟宜机场"},
    "Hong Kong": {"code": "HKG", "name": "Hong Kong International Airport", "name_cn": "香港国际机场"},
    "New York": {"code": "JFK", "name": "John F. Kennedy International Airport", "name_cn": "纽约肯尼迪国际机场"},
    "Los Angeles": {"code": "LAX", "name": "Los Angeles International Airport", "name_cn": "洛杉矶国际机场"},
    "London": {"code": "LHR", "name": "London Heathrow Airport", "name_cn": "伦敦希思罗机场"},
    "Paris": {"code": "CDG", "name": "Paris Charles de Gaulle Airport", "name_cn": "巴黎戴高乐机场"},
}
...
def format_flight_table(flights: list[dict]) -> str:
    ...
    lines = []
    lines.append("=" * 90)
    lines.append(f"{'Flight':<10} {'Departure':<18} {'Arrival':<18} {'Duration':<10} {'Price':<12} {'Stops'}")
    lines.append("=" * 90)
...
def main():
    parser = argparse.ArgumentParser(description="Search for available flights (mock data)")
    parser.add_argument("--from", dest="from_city", required=True, help="Departure city")
    ...
    parser.add_argument("--class", dest="cabin_class", default="economy", 
                        choices=["economy", "business", "first"], help="Cabin class")
    ...
    print(f"Date: {args.date} | Passengers: {args.passengers} | Class: {args.cabin_class.capitalize()}")
    print()
    
    # Generate outbound flights
    outbound_flights = generate_mock_flights(
        args.from_city, args.to_city, args.date,
        args.passengers, args.cabin_class
    )
    ...
    if args.json:
        ...
        if args.return_date:
            return_flights = generate_mock_flights(
                args.to_city, args.from_city, args.return_date,
                args.passengers, args.cabin_class
            )
