...
AUTHORITY_TIERS = {
    ...
    4: ["reddit.com", "quora.com", "twitter.com", "linkedin.com"],
}
    ...
    comparison_result = {
        ...
        "sources": [
            ...
            for s in sorted(
                sources,
                key=lambda x: (
                    -x.authority_tier if evaluate_authority else 0,
                    -x.score,
                ),
            )
        ],
...
def analyze_consensus(sources: list[SourceInfo]) -> dict:
    ...
    if len(sources) == 1:
        return {
            ...
            "message": "Only one source found. Verify with additional sources.",
        }
...
def get_authority_recommendation(sources: list[SourceInfo]) -> str:
    ...
    if tier_1 >= 2:
        ...
        return (
            "Good source quality. Mix of primary and authoritative secondary sources."
        )
    elif tier_2 >= 2:
...
def format_output(result: dict, output_format: str) -> str:
    ...
    for i, source in enumerate(result["sources"][:5], 1):
        tier_str = (
            f" [Tier {source['authority_tier']}]"
            if source.get("authority_tier")
            else ""
        )
        lines.extend(
            [
...
def main():
    ...
    parser.add_argument(
        "--evaluate_authority", action="store_true", help="Include authority assessment"
    )
