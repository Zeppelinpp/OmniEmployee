...
def validate_claim(
    ...
    for result in results:
        ...
        if any(word in content for word in ["confirms", "verified", "true", "correct", "accurate"]):
            supporting.append(source_info)
        elif any(word in content for word in ["false", "incorrect", "debunked", "myth", "wrong"]):
    ...
    validation_result = {
        ...
        "recommendation": get_recommendation(len(supporting), len(contradicting), total_sources)
    }
...
def main():
    parser = argparse.ArgumentParser(description="Validate a claim across multiple sources")
    parser.add_argument("--claim", required=True, help="The claim to validate")
    parser.add_argument("--min_sources", type=int, default=3, help="Minimum sources needed for validation")
